...
public class RandomOptimizer {
	 ...
	 **/
	public Operator getOptimizedPlan2() {
		...
		// initial plan & cost
		Operator plan = rip.prepareInitialPlan();
		...
		// initial startin temperature
		int temperature = 2 * cost;
		// num of iteration to get another neighbor plan after reduction
		int equilibriumCounter = numJoin * 20;
		// keep track of best cost, min
		int bestCost = cost;
		// the final plan, that should be have the best cost
		Operator finalPlan = plan;
		if (numJoin != 0) {

			// while temperature not frozen
			while (temperature > 0) {

				// while not equilibrium
				for (int i = 0; i < equilibriumCounter; i++) {
					Operator planCopy = (Operator) plan.clone();
					cost = pc.getCost(plan);
					Operator neighbor = getNeighbor(planCopy);

					// calculate the cost for neighbor plan
					int changeCost = pc.getCost(plan) - pc.getCost(neighbor);

					// if neighbor has better cost
					// or neighbor has worst cost, still allow some chance of
					// exploration
					if (/* better cost */changeCost < 0 || /* worse cost */ Math.random()/* probability */ <= Math
							.pow(Math.E, -changeCost / temperature)) {
						plan = neighbor;
						cost = pc.getCost(plan);
					...
					// overwrite best plan if plan found is better
					if (cost < bestCost) {
						bestCost = cost;
						finalPlan = plan;
					}
	...
	public static Operator makeExecPlan(Operator node) {
		...
		if (node.getOpType() == OpType.JOIN) {
			...
			/**
			 * Temporarity used simple nested join, replace with hasjoin, if
			 * implemented
			 **/
